...
async function filesIn(dir) { try { return await readdir(dir); } catch { return []; } }
...
function trimEffectEntries(list, locs) {
  ...
  for (const e of list) {
    ...
    if (!ln || (!langs.has(ln) && ln !== 'en')) continue;
    const copy = { ...e };
...
function flavorTextFor(species, loc) {
  ...
  if (want === 'ja') {
    ...
    if (ja) return String(ja).replace(/[\n\f]/g, ' ');
    const jaHrkt = list.find((e) => e?.language?.name === 'ja-Hrkt')?.flavor_text;
    if (jaHrkt) return String(jaHrkt).replace(/[\n\f]/g, ' ');
  }
...
function collapseVersions(p) {
  ...
  const CAT_KEYS = [
    'front_default','back_default','front_shiny','back_shiny','front_female','back_female','front_shiny_female','back_shiny_female'
  ];
  for (const gen of GEN_ORDER) {
    ...
    for (const key of CAT_KEYS) {
      ...
      if (typeof gobj?.[key] === 'string') url = gobj[key];
    ...
    if (Object.keys(acc).length > 0) out[gen] = acc;
  }
  return out;
}
    ...
    for (const vg of mv?.version_group_details || []) {
      ...
      if (!m.has(key)) {
        ...
        if (method === 'level-up') {
          ...
          if (lvl != null && lvl !== 0 && (existing.level == null || existing.level === 0 || lvl < existing.level)) existing.level = lvl;
        }
        const nm = vg?.version_group?.name;
        if (nm && !existing.versionGroups.includes(nm)) existing.versionGroups.push(nm);
      }
    }
  }
  const sections = [];
  for (const [gen, info] of genMap) {
    ...
    for (const [method, items] of info.methods) {
      const list = Array.from(items.values()).sort((a, b) => {
        if (method === 'level-up') {
          const lvA = a.level ?? 0, lvB = b.level ?? 0;
          if (lvA !== lvB) return lvA - lvB;
        }
        return a.move.name.localeCompare(b.move.name);
      });
      if (list.length) methods.push({ method, items: list });
    }
...
async function main() {
  ...
  if (LIMIT != null) console.log(`Pokémon limit: ${LIMIT}`);
  ...
  let evoChains = SOURCE !== 'pokeapi' ? await readAggregated('evolution-chain') : undefined;
  ...
  if (needFetch) {
    ...
    species = speciesData; pokes = pokesData; moves = moveData; abilities = abilityData; types = typeData; growthRates = growthData; colors = colorData; eggGroups = eggGroupData;
  }
  ...
  for (const p of pokes || []) {
    ...
    if (p?.id != null && sid != null) pokemonToSpecies.set(p.id, sid);
  }
  ...
  async function evolutionChainForSpeciesId(id) {
    ...
    if (evoChainCache.has(cid)) return evoChainCache.get(cid);
  ...
  for (const loc of LOCALES) {
    ...
    for (const s of species) {
      const id = s.id;
      if (LIMIT != null && id > LIMIT) continue; // rough guard in dev mode
  ...
  function collectAliasesFromNames(item) {
    ...
    for (const loc of LOCALES) {
      ...
      if (want === 'ja') {
        const ja = names.find((n) => n.language?.name === 'ja')?.name;
        if (ja) set.add(String(ja));
        const jaHrkt = names.find((n) => n.language?.name === 'ja-Hrkt')?.name;
        if (jaHrkt) set.add(String(jaHrkt));
      } else {
  ...
  try {
    const map = {};
    for (const s of species || []) {
      const id = s?.id; if (id == null) continue;
  ...
  for (const [res, all] of [ ['move', moves], ['ability', abilities], ['type', types] ]) {
    try {
      ...
      for (const item of all || []) {
        const id = item?.id; if (id == null) continue;
  ...
  try {
    ...
    for (const m of moves || []) {
      for (const pk of (m?.learned_by_pokemon || [])) {
        ...
        if (id != null) learnerPokemonIds.add(id);
      }
    }
    ...
    if (toResolve.length) {
            ...
            if (sid != null) {
              if (!speciesById.has(sid)) {
                try {
                  ...
                  if (s?.id != null) speciesById.set(s.id, s);
                } catch {}
              }
              return [pid, sid];
            }
      ...
      for (const [pid, sid] of resolved) if (sid != null) pokemonToSpecies.set(pid, sid);
    }
  } catch (e) {
  ...
  for (const loc of LOCALES) {
    ...
    for (const s of species) {
      const id = s.id;
      if (LIMIT != null && id > LIMIT) continue;
      const p = pokeById.get(id);
      if (!p) continue;
      ...
      const detail = {
        ...
        species: {
          ...
          egg_groups: Array.isArray(s?.egg_groups) ? s.egg_groups.map((g) => {
