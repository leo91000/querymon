...
function kg(weightHectograms: number) {
    return (weightHectograms / 10).toFixed(1);
}
function m(heightDecimeters: number) {
    return (heightDecimeters / 10).toFixed(1);
}
...
function findLocalFlavor(species: Species, lang: Locale): { text?: string; hasWanted: boolean } {
    ...
    if (!list)
        return { text: undefined, hasWanted: false };
    const wanted = lang === 'jp' ? ['ja', 'ja-Hrkt'] : [lang];
    const foundWanted = list.find(e => wanted.includes(e?.language?.name || ''));
    if (foundWanted?.flavor_text)
        return { text: String(foundWanted.flavor_text).replace(/[\n\f]/g, ' '), hasWanted: true };
...
export default function PokemonDetail(props: { id: number }) {
    ...
    // official art URL is derived in the sprite viewer/component when needed
                    ...
                    <>
                        ...
                        <Card>
                            ...
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                <For each={[...Array.from({ length: 8 }).keys()]}>
                                    {() => (
                                        <div>
            ...
            >
                <Card class="relative overflow-hidden p-0">
                    <div class="absolute right-3 top-3 z-0 md:right-4 md:top-4">
                        <Tooltip placement="bottom" content={isFavorited() ? (t('common.removeFavorite') || 'Remove favorite') : (t('common.addFavorite') || 'Add favorite')}>
                            <button
                        ...
                        <div class="p-6">
                            <div class="flex items-center gap-3">
                                <h2 class="text-2xl font-bold tracking-tight">
                                    <span class="mr-2 font-jersey text-blue-600 dark:text-blue-400">
                                        #
                            ...
                            <div class="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3">
                                <div class="rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700">
                                    <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                                        <span class="icon-[ph--scales] text-blue-600 dark:text-blue-400" />
                                        {' '}
                                        {t('pokemon.weight')}
                                    </div>
                                ...
                                <div class="rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700">
                                    <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                                        <span class="icon-[ph--ruler] text-blue-600 dark:text-blue-400" />
                                        {' '}
                                        {t('pokemon.height')}
                                    </div>
                                ...
                                <div class="rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700">
                                    <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                                        <span class="icon-[ph--target] text-blue-600 dark:text-blue-400" />
                                        {' '}
                    ...
                    <Card>
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                                ...
                                <div class="font-medium">
                                    {(() => {
                                        const hdm = pokemon()?.height ?? 0;
                                        const m = (hdm / 10).toFixed(1);
                            ...
                            <div>
                                ...
                                <div class="font-medium">
                                    {(() => {
                                        const arr = speciesData()?.egg_groups || [];
                                        const names = arr.map(g => String(g?.name || '')).filter(Boolean);
                                        return names.join(', ') || '—';
                                    })()}
                                </div>
                            </div>
                                ...
                                <div class="font-medium">
                                    {(() => {
                                        ...
                                        if (gr == null)
                                            return '—';
                                        if (gr === -1)
                                            return t('pokemon.genderless');
                                        const female = (gr * 12.5).toFixed(1);
                                        const male = (100 - gr * 12.5).toFixed(1);
                                        return `${female}% ${t('pokemon.female')} ; ${male}% ${t('pokemon.male')}`;
                                    })()}
                                </div>
                            </div>
                            <div>
                                    ...
                                    {(stage, idx) => (
                                        <div class="flex items-center gap-6">
                                                    ...
                                                    <For each={stage}>
                                                        {(entry) => {
                                                            const hints = createMemo(() => {
                                                                const _ = getLocale();
                                                                return summarizeEvolutionDetails(entry.details);
                                                            });
                                                            return (
                                                                <a
                                            ...
                                            <Show when={idx() < evolutionStages().length - 1}>
                                                <span class="icon-[ph--arrow-right-bold] hidden text-3xl text-gray-300 dark:text-gray-600 md:block" />
                                            </Show>
                                        </div>
                                    )}
                                </For>
