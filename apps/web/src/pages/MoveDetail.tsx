                        ...
                        <div class="grid grid-cols-1 md:grid-cols-[1fr_240px]">
                            ...
                            <div class="relative flex items-center justify-center p-4">
                                <div class={`flex h-20 w-20 items-center justify-center rounded-full text-white ring-4 ${typeToneBg(typeName())}`}>
                                    <span class={`icon-[${typeIconClass(typeName())}] text-4xl`} />
                                </div>
                            </div>
                        </div>
                    </Card>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <Card>
                            ...
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                ...
                                {(() => {
                                    const v = m().meta?.drain;
                                    return <MetaRow label={t('move.drain')} value={v != null ? `${v}%` : '0%'} />;
                                })()}
                                {(() => {
                                    const v = m().meta?.healing;
                                    return <MetaRow label={t('move.healing')} value={v != null ? `${v}%` : '0%'} />;
                                })()}
                                <MetaRow label={t('move.flinchChance')} value={`${m().meta?.flinch_chance ?? 0}%`} />
                                <MetaRow label={t('move.effectChance')} value={`${m().effect_chance ?? 0}%`} />
                                {(() => {
                                    const min = m().meta?.min_hits;
                                    const max = m().meta?.max_hits;
                                    return <MetaRow label={t('move.hits')} value={min ? `${min}-${max ?? min}` : '—'} />;
                                })()}
                                {(() => {
                                    const min = m().meta?.min_turns;
                                    const max = m().meta?.max_turns;
                                    return <MetaRow label={t('move.turns')} value={min ? `${min}-${max ?? min}` : '—'} />;
                                })()}
                            </div>
