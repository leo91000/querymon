...
export default function PokemonLearnset(props: PokemonLearnsetProps) {
    ...
    const idsByGeneration = createMemo(() => {
        if (props.learnsets && props.learnsets.length > 0)
            return new Map<GenerationSlug, Set<number>>();
    ...
    const learnset = createMemo(() => {
        ...
        if (props.learnsets && props.learnsets.length > 0) {
            ...
            for (const sec of props.learnsets) {
                ...
                for (const ent of sec.entries) {
                    ...
                    if (items.length > 0)
                        entries.push({ method, items });
                }
                if (entries.length > 0)
                    out.push({ generation: gen, entries, order: (sec as any).order ?? 0 });
            }
        ...
        if (!moves.length)
            return [] as Array<{ generation: GenerationSlug; entries: Array<{ method: MethodKey; items: LearnsetEntry[] }>; order: number }>;
        ...
        for (const move of moves) {
            ...
            for (const vg of move?.version_group_details || []) {
                ...
                if (!methodMap.has(key)) {
                    ...
                    if (vgName && !existing.versionGroups.includes(vgName))
                        existing.versionGroups.push(vgName);
                }
            }
        }
                        ...
                        {(section) => {
                            ...
                            return (
                                <button
                                    ...
                                    type="button"
                                    class={`cursor-pointer rounded-t-md px-3 py-2 text-sm font-medium transition focus:outline-none ${selected()
                            ...
                            <div class="space-y-6">
                                <For each={sec().entries}>
                                    {(methodSection) => {
                                        ...
                                        const recalcM = () => {
                                            if (methodOpen())
                                                queueMicrotask(() => setMh(`${methodRef?.scrollHeight ?? 0}px`));
                                            else setMh('0px');
                                        };
