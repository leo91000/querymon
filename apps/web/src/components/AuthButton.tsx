...
import { authClient, type Session } from '../services/authClient';
...
export default function AuthButton() {
  ...
  return (
    <div class="flex items-center gap-2">
      <Show when={!loading()}>
        <Show when={(session() as any)?.user || (session() as any)?.data?.user} fallback={
          <button
          ...
          <button
            class="rounded-full border border-gray-200 px-3 py-1 text-sm hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700/50"
            onClick={async () => { await authClient.signOut(); setSession(null); }}
          >
            Sign out
          </button>
        </Show>
