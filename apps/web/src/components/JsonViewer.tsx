...
interface NodeProps { keyName?: string; value: any; level?: number }

function isObject(x: any) {
    return x && typeof x === 'object' && !Array.isArray(x);
}

function Toggle(props: { open: boolean; onClick: () => void }) {
    return (
        <button
            ...
            aria-label={props.open ? t('json.collapse') : t('json.expand')}
        >
            {props.open ? '−' : '+'}
        </button>
    );
...
function Node(props: NodeProps) {
    ...
    const entries = () => (isObject(v()) ? Object.entries(v() as Record<string, any>) : []);

    return (
        <div class={`border-l border-gray-200 dark:border-gray-700 ${pad()}`}>
            <Show when={Array.isArray(v())}>
                <div>
                    {k() && (
            ...
            <Show when={!Array.isArray(v()) && !isObject(v())}>
