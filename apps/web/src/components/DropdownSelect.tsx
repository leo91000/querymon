...
export default function DropdownSelect<T extends string>(props: DropdownSelectProps<T>) {
  ...
  function onPointerDown(e: PointerEvent) {
    ...
    if (!rootRef.contains(target) && !(listEl && listEl.contains(target))) close();
  }
  ...
  function reposition() {
    ...
    queueMicrotask(() => {
      ...
      let t = top;
      if (placeAbove) t = Math.max(8, r.top - ph - margin);
      let l = left;
      if (props.align !== 'left') l = Math.max(8, r.right - pw);
      ...
      <button
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'ArrowDown') {
            ...
            else { openMenu((active() + 1) % len); reposition(); }
          } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            if (open()) setActive((a) => (a - 1 + len) % len);
            else { openMenu((active() - 1 + len) % len); reposition(); }
        ...
        <Portal>
          <ul
            ...
            onFocusOut={(e) => {
              ...
              const listEl = listRef as unknown as Node | null;
              if (next && (rootRef?.contains(next) || (listEl && listEl.contains(next)))) return;
              close();
            }}
            onKeyDown={(e) => {
              const len = props.options.length;
