...
export async function loadList(resource: ResourceName): Promise<ListItem[]> {
    ...
    if (mapped) {
        ...
        try {
            return await ensure(['list', resource, loc, 'v1'], () => fetchJSON<ListItem[]>(url));
        }
        catch { }
    }
    ...
    for (const url of candidates) {
        try {
            return await ensure(['listLegacy', resource, loc, url], () => fetchJSON<ListItem[]>(url));
        }
        catch { /* try next */ }
    }
...
export async function loadItemById<T = any>(resource: ResourceName, id: number): Promise<T | undefined> {
    ...
    if (['pokemon', 'move', 'ability', 'type'].includes(resource)) {
        ...
        try {
            return await ensure(['item', resource, id, loc, 'v1'], () => fetchJSON<T>(perLocale));
        }
        catch { }
    }
...
export async function loadAliases(resource: 'pokemon' | 'move' | 'ability' | 'type'): Promise<Record<string, string[]>> {
    try {
        return await ensure(['aliases', resource, 'v1'], () => fetchJSON(`${BASE}/aliases.${resource}.json`));
    }
    catch { return {} as any; }
}
...
export async function loadTypeEntries(): Promise<any[]> {
    ...
    if (!typeEntriesPromise)
        typeEntriesPromise = ensure<any[]>(['legacy', 'type.json'], () => fetchJSON<any[]>(`${BASE}/type.json`)).catch(() => []);
    return typeEntriesPromise;
}

let growthRatesPromise: Promise<any[]> | undefined;
export async function loadGrowthRates(): Promise<any[]> {
    if (!growthRatesPromise)
        growthRatesPromise = ensure<any[]>(['legacy', 'growth-rate.json'], () => fetchJSON<any[]>(`${BASE}/growth-rate.json`)).catch(() => []);
...
export async function loadGrowthRatesLite(loc?: Locale): Promise<Array<{ id: number; name: string; exp100: number | null }>> {
    const locale = loc || (getLocale() as Locale);
    try {
        return await ensure(['growth-rates', locale, 'v1'], () => fetchJSON(`${BASE}/growth-rates.${locale}.json`));
    }
    catch { return []; }
}
