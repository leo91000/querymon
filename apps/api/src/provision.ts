...
export interface ProvisionInfo { dbName: string; dbUrl: string; dbHost: string; token?: string }
...
export async function provisionForUser(userId: string, rootDb: DB): Promise<ProvisionInfo> {
    ...
    const hostname = (() => {
        if (details && typeof details === 'object') {
            ...
            if (typeof hn2 === 'string')
                return hn2;
        }
        return undefined;
    })();
    if (!hostname)
        throw new Error('turso db hostname not found');
...
async function maybeIssueDbToken(dbName: string, env: z.infer<typeof Env>): Promise<string | undefined> {
    if (!env.TURSO_API_TOKEN || !env.TURSO_ORG_SLUG)
        return undefined;
